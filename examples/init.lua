...
-- 大文字小文字は問いません）。中身を見るのに展開が要る重いもの向けです。
-- F3 では開けます — これは「頼んでいないのに開く」のを止める設定です。
...
--   :ragdebug [質問]  RAG がどの断片をどのスコアで拾ったかを表示（足切りなしの生スコア）。
--                  引数を省くと直前の :rag の質問について。チャット中は Ctrl+D でも。
--                  回答が的外れなとき、拾った断片が悪いのかモデルが悪いのかを切り分けます。
