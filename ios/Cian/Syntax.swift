...
/// マークダウンの書き方（iPhone）。
///
/// **顔ぶれはデスクトップ版と同じ**（`gui/renderer.js` の `cmdSyntax`）── 記号の説明が
/// 端末によって違うと、片方で覚えたことがもう片方で通じない。
///
/// iPhone では**入れる**のではなく**見せる**だけにした。デスクトップ版は「選ぶと、いま
/// 書いているところに入る」が、設定の奥から辿ってくるiPhone では、そこへ
/// 戻ってくる頃には打っていた場所を離れている ── 記号そのものは下の帯の
/// ボタンから入る（そちらがiPhone の入口）。
struct Syntax: View {
...
/// amber について（iPhone）。
///
/// **不具合を伝えるときの3 つ**（デスクトップ版の `cmdAbout` と同じ顔ぶれ）── 画面の
...
struct About: View {
    ...
    /// した: あれはアプリ中のボタンを塗る色で、ここだけ変えたいときに
    /// 一緒に動いてしまう。
    ...
    var body: some View {
        List {
            ...
            // 名前の意味はメニューにも帯にも出さない ── 一度読めば済むもので、
