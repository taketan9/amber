...
/// **よその予定表**（依頼 456・iPhone の側）。
///
/// Google カレンダーの非公開 URL も、iCloud の公開リンクも、中身は同じ形
/// （iCal）── 読むのは核（`ics`）で、**デスクトップ版と同じ一組**。同じ URL を貼れば、
/// デスクトップ版と iPhone に同じ予定が同じ日に並ぶ。
///
/// **アドレスは、この iPhone の中だけに持つ。** それを知っている人が予定を
/// 全部読めるので、キーと同じ扱い ── ノートにも `.amber/` にも書かない
/// （あそこはフォルダと一緒に旅をする）。
@MainActor
enum Away {
    ...
    ///
    /// **一つ取れなくても、ほかは出す。** ネットワークの向こうの都合で全部が出ない
