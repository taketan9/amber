...
struct Making: View {
    ...
    let stencils: [Note]
    /// 型が選ばれたとき。ここで作って、このダイアログは閉じる。
    let fromStencil: (Note) -> Void
    /// 型が一つも無いとき、サンプルの三枚（週報・議事録・買い物リスト）を入れる（依頼 506）。
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                // 探しているのがまさにこれで、メニューの奥に置くと見つからない。
                // 選んだ瞬間に作る ── 型を選んでから題とタグを訊くと、
                // 型の中に書いてあるものをもう一度訊くことになる。
                Section("テンプレートから") {
                    if stencils.isEmpty {
                        // **無いなら、その場で入れられる**（依頼 506）── 段ごと隠すと、
                        // テンプレートというパスがあることに気づけない。
                        if let seedStencils {
                            Button { seedStencils() } label: {
                                Label("サンプルのテンプレートを入れる（週報・議事録・買い物リスト）", systemImage: "doc.on.doc")
                            }
                        }
