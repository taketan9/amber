...
struct Studio: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle(byText || model == nil ? "コードで直す" : "図で直す")
            ...
            .toolbar {
                ...
                if model != nil {
                    ToolbarItem(placement: .bottomBar) {
                        Button(byText ? "図で直す" : "コードで直す") { swap() }
                    }
                }
            }
        }
    ...
    private var form: some View {
        if byText || model == nil {
            VStack(alignment: .leading, spacing: 8) {
                if model == nil {
                    Text("この図は表にできない形（手で書いたか、ambər の知らない書き方）です。上の図を見ながら、ここで直してください。")
    ...
    /// 線は**二段**にする。402pt に「ここから・言葉・ここへ」を横に3 つ
