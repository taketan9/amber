...
/// 絞り込みの帯（iPhone）── **デスクトップ版と同じ形**（`gui/renderer.js` の引き出し）。
///
/// タグ・フォルダ・期間の3 つが一覧の頭に常に並び、いくつでも重なる。
/// **絞れることが、絞る前から見えている** ── 「フィルタ」という言葉を
/// 覚えなくてよい。押すと、そのすぐ下に選ぶ画面が開く。
///
/// 重なり方は種類で違う: **タグは全部・フォルダはどれか**。ノートは一つの
/// フォルダにしか居ないので、フォルダを「全部」にすると二つ選んだ瞬間に
/// 必ず 0 件になる。どちらなのかは開いた画面に書く。
struct Sifting: View {
    ...
    /// 帯の一つに出す文字。**選んだものを、開かずに読ませる** ──「タグ 1」では
...
/// カレンダー ── **いつからいつまでを、押して決める**（デスクトップ版と同じ）。
///
