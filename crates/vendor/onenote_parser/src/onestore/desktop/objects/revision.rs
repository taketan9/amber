...
fn parse<'a>(
    ...
    while let Some(current) = iterator.peek() {
        if let FileNodeData::RevisionManifestEndFND = current {
            ...
            // ambər の直し（依頼 636）── **どこに出てきても飛ばす。**
            //
            // 元は、全体 ID 表（GlobalIdTable）のすぐ後ろに続くぶんだけを
            // 飛ばしていた。本人の会社の `.one` は、それ以外の場所にもこの
            // 節を持っていて、「Unexpected node (parsing Revision)」で
            // 一冊まるごと読めなかった（現場から文言をもらった）。
            //
            // この節が持つのは**参照の数え**（MS-ONESTORE 2.5.20）で、
            // 中身を読むのに要る値は一つも入っていない ── 飛ばして困らない。
            iterator.next();
        } else {
            return Err(
