...
//! `.買い物リスト.md.icloud` という**プレースホルダ**を置く。名前が違うので amber の
...
//! 作るが、「週報 2」という**本物のノート**と名前の上で見分けが付かない。
...
pub enum Kind {
    /// まだダウンロードされていない ── 置いてあるのは中身ではなくプレースホルダ。
    Waiting,
    /// 同時に書いたので、クラウドが作った控え。
    Clash,
}
...
///
/// 渡すのはファイル名だけ（パスではなく）。判断にパスは要らないし、パスを見ると
/// フォルダの名前に釣られる。
...
/// 見ないのでプレースホルダが 1 つも出てこないが、**そのときに見つけたフォルダは分かって
/// いる** ── その一段ずつを覗くだけで足りる。木をもう一度下りる必要は無い。
///
/// 衝突の控えはこちらでは拾わない ── あちらは隠れておらず、ノートとして
/// 一覧に出ている。**一覧から消さずに印を付ける**のが正しい（消すと、中身を
/// 取り戻す手段がどこにも無くなる）。
/// 返すのは**本来のノートのパス**（プレースホルダのパスではなく）── iPhone はこれを iOS に
/// 渡して「ダウンロードして」と頼む（`startDownloadingUbiquitousItem`）。プレースホルダのパスを
/// 渡しても、あちらは何のことか分からない。
pub fn waiting(root: &std::path::Path, rows: &[crate::survey::Row]) -> Vec<std::path::PathBuf> {
    ...
    for r in rows {
        ...
        // クラウドのプレースホルダが出ることはあるが、それはノートの話ではない。
