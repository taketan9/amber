...
/// 同じ問いの裏表で、別々に書くと **片方が見落とした画像を、もう片方が
...
/// 入る（既定のノートは `2026-09-06 19-18-30.md` で、その画像は
...
mod title_tests {
    ...
    /// **空白で切ると、指されている画像を「使われていない」と言う。**
    /// amber の既定のノートは `2026-09-06 19-18-30.md` で、その画像は
    /// 空白を二つ含む ── 人はその一覧を見て消す。
    #[test]
    fn 空白を含む名前の画像を_見失わない() {
