 ...
 *
 * 注記の見出し（NOTE・TIP・…）の名前が、**核と窓で同じか。**
 *
 * **実際にずれていた。** 核と窓は「ノート／こつ／大事／注意／危険」、電話
 * だけ「おぼえておく／こつ／大事／注意／あぶない」── 誰も気づかないまま
 * 置いていかれていた（本人が言葉を直そうとして見つかった）。
 *
 * **2026-09-16、電話の表は無くなった**（依頼 609）── あれは死んだ画面
 * （`Reading`）の中にあり、電話の「表示」の面は前から核の `to_html` を
 * そのまま出している。**三つ目の表が無いほうが強い約束**なので、
 * 「電話は自分の表を持たない」を検査のほうに移した ── また生えたら鳴る。
 *
 * 見るのは形ではなく**字そのもの**。増やすときは、二か所とも直すことになる。
 */
'use strict';
const fs = require('fs');
...
/// **電話は、自分の表を持たない。**
///
/// 「表示」の面は核の `to_html` をそのまま出す ── 名前を Swift でもう一度
...
const three = { 核: fromCore(), 窓: fromWindow() };
let bad = 0;
for (const k of KINDS) {
...
if (own.length) {
    bad += 1;
    console.log('✗ 電話が自分の見出しの表を持っています: ' + own.join(' / '));
    console.log('  表示の面は核の to_html を出します ── 名前をもう一度書くと、');
