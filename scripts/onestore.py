...
こちらでは直せなかった。**書き出したファイルを直に読む**のがこのファイル。
...
def _color(v):
    ...
    3 つは `0x00`）。「自動」は「黒を指定した」ではなく「指定していない」
    なので、そこに色を書くと**ノートの全部の行が span に包まれる。**
    最後が `0x00` のときだけ、前の3 つが赤・緑・青。
    """
...
def by_oid(objs):
    """OID から実体を引く 1 つ（参照をたどるのに要る・依頼 608）。"""
...
def table_texts(d, objs):
    ...
    本文に二度出さないための 1 つ ── セルをたどるときに拾うので、ここで
    拾うと同じ文字が表の外にも並ぶ。
    """
    look = by_oid(objs)
    inside = set()
...
def _empty(pg):
    """**空っぽの 1 つか。** 題も本文も表も絵も無ければ、写しても何も残らない。"""
