#!/usr/bin/env node
/*
 * **組んだ zip が、本当に読めるか**（依頼 550）。
 *
 *     node scripts/zip-test.js
 *
 * `scripts/zip.js` は配るものを組む唯一の場所で、**壊れても組んだ側には
 ...
 *   三. 中身が一字も変わっていない（CRC と、読み直した字そのもの）
 ...
 * 黙る（実際に、表を潰しても鳴らなかった）。こちらは表を作らず一ビットずつ
...
// 日本語の名前、よく縮む字、縮まない出鱈目、空の一枚 ── 四つとも通り道が違う。
...
// 記号リンク ── **持っていかない**（Windows に置けない）。作れない機械
...
if (end >= 0) {
    ...
    for (let i = 0; i < count; i += 1) {
        ...
        // 本体の側も読む ── 位置が一つでもずれていれば、ここで印が合わない。
        ...
        found.set(name, {
            utf8: !!(flags & 0x800),
            // **局所ヘッダの印も見る。** 展開する側は、名前をこちらから読む
            // ことがある ── 中央目録にだけ印を立てても、化ける相手が残る
...
ok(!!got && got.utf8Local, '局所ヘッダにも UTF-8 の印が立っている');
...
ok([...found.values()].every((f) => f.text !== null && crcSlow(f.text) === f.sum),
   'CRC が中身と合っている');
ok(!!squash && squash.csize < squash.usize, 'よく縮む字は縮んでいる');
...
ok(!!empty && empty.usize === 0, '空の一枚も入る');
