 ...
 * `var(--ink3)` と書いたところは（本当は `--ink-3`）、文字の色なら
 ...
 *
 * カレンダーには見方が五つある（月のセル目・週と日の帯・終日の段・
 * グループの帯・グループのセル目）。色は五か所に別々に書いてあるので、
 * 種類を一つ足すと**どこかを書き忘れる** ── 実際、チームの紫を足した
 * とき、月のセル目と平の週表の二か所が琥珀のまま残っていた。同じ予定が
 ...
 * 思ってもらえない ── 実際、カレンダーの予定も「使われていない画像」のセルも
 * 押せるのに、乗せても何も起きなかった。
 *
 * **`#sheet .it` はここで数えない** ── あれはキーボードと同じ選び目を動かすので、
