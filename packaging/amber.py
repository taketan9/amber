...
def ico(pngs):
    ...
    こちらは生成できる。8行の仕様（`.ico` のヘッダ）で、判断は入っていない。
    """
    n = len(pngs)
...
def render(size, square=False):
    ...
    # 小さいサイズでは尻尾も文字も潰れる。線を細らせず、図形ごと少しだけ太らせる
...
def agree():
    ...
    **見張る先は二か所に減った。** 以前は四か所 ── ここ、`amber.svg`、
    ...
    中のマークだけが葉のまま残った。** 見張れていたのは「四つのコピーが揃っているか」
    であって、「アイコンと同じか」ではなかった ── 揃ったまま、全部が古く
    なれる。いまはデスクトップ版も iPhone も `packaging/amber-mark.png`（アイコンを
    小さくしたもの）を出すので、コピーは無く、ずれようがない。
...
if __name__ == '__main__':
    # **アイコンの生成は `amber_icon.py` に移った。**
    #
    # 案 S4 の葉はアプリのマークではなくなり、いまのアイコンは案2「琥珀の中の
    # Markdown」── 画素そのものを原本として持つので、ここでは生成できない。
    # デスクトップ版と iPhone の中のマークも、いまは `packaging/amber-mark.png`（アイコンを
    # 小さくしたもの）を出す。**この file と `amber.svg` は案 S4 の記録**で、
