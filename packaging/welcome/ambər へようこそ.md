...
絵はノートの**隣の `attachments/` に置かれ**、本文にはリンクだけが入ります
── ノートを別の場所へ移すと、絵も一緒に付いていきます。

窓は**貼り付け**（⌘V）でそのまま入ります。iPhone は下の道具の帯の 🖼 から。

大きさを変えるには、**絵を押して「大きさ…」**。小さめ・中くらい・大きめ・
はばいっぱいから選べます。
選んだものは題のところに `![w:200px](attachments/amber.png)` と**字で書かれる**ので、
