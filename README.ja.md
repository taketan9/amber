...
- **カーソル追従プレビュー（既定オン）。** シェル枠の領域がカーソル位置のものをプレビューします — コードは色つき、画像、フォルダと書庫の一覧、Office/PDF のテキスト。シェルは下で動き続けていて、`Shift+J` かクリックで戻ります。リモートペインではあえてプレビューしません（触れるたびダウンロードになるため）。`:preview` でオフ。展開やスキャンをしないと何も出せない種類は**既定で対象外**です — `.vsix` `.jar` `.whl` `.iso` `.msi` `.mdf` など。`.zip` や `.tar` は対象外にしていません（中を見るために開くので）。`cian.set_option("preview_skip", { "bak" })` で追加できます。`F3` ではどれも開けます。
