...
ビューアは vim 風：`h j k l`・`w b`・`0 $`・`gg G`・`Ctrl-d/u` で移動、`/` で検索、 `42G` で行ジャンプ、`%` で対応する括弧へ。`v` / `V` / `Ctrl-v` で選択、`y` でコピー。文字化けしていたら `e` で文字コード切替（UTF-8 / Shift_JIS / UTF-16）。

**その場で編集：** `i` でそのまま編集モード（`Ctrl+S` で元の文字コードのまま保存、 `Esc` で抜ける）。自分のエディタがいい？ **`E`**（または `:edit`）で `$VISUAL` / `$EDITOR`（無ければ nvim → vim → vi）を開き、戻ってきたら再読み込みします。
