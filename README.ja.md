...
| `?` | ビューア専用のキー一覧（用途別） |
| **Tab** | 隣のペインへ移動／戻る — 一覧でも、開いている別のファイルでも。シェルには行きません（パネルからは `Shift+K`、一覧からは `Shift+J`） |
