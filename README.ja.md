...
**見えない文字。** TAB・半角空白・全角空白・改行コードを、それぞれ `→` `·` `□` `↓`／`↵↓` で描きます。何も無いように見えて厄介ごとを起こすのは、まさにこれらなので。`:ws` で消せます。タブ幅は既定で4桁、`cian.set_option("tab_width", 8)` で広げられます。タブ区切りのファイルが揃うかどうかはここで決まります（4桁だと、4文字のフィールドはちょうど1桁ぶんを埋めきってタブが次の停止位置へ進むので、続く列は8桁目から始まります。一方2桁の `あ` は4桁目から。これはタブの仕様であって、直すべき不具合ではありません）。
