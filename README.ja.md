...
- **カーソル追従プレビュー（既定オン）。** シェル枠の領域がカーソル位置のものをプレビューします — コードは色つき、画像、フォルダと書庫の一覧、Office/PDF のテキスト。シェルは下で動き続けていて、`Shift+J` かクリックで戻ります。リモートペインではあえてプレビューしません（触れるたびダウンロードになるため）。`:preview` でオフ。`cian.set_option("preview_skip", { "vsix", "iso" })` で拡張子ごとにも切れます — 中身を見るのに展開が要る重いもの向けです。`F3` では開けます。
