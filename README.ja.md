...
|---|---|
| **← / →** | ペイン間でフォーカス移動 |
| **Enter** | フォルダ・書庫に入る／**ファイルはその場で読む** — 一覧が本文に変わり、`j`/`k`/`Space`/`G` でスクロール、`Esc` で一覧に戻る、`F3` でエディタへ |
...
`F3`（と Shift+Tab）は**エディタ**を開き、画面を占有します。`Enter` は**ペインの中で読む**方です — 一覧がファイルの中身に変わり、反対のペインはそのまま、`Esc` で元の行に戻ります。以下はエディタの話です。
