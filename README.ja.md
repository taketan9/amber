...
vim 風です：`h j k l`・`w b`・`0 $`・`gg G`・`Ctrl-d/u`、`%` で対応する括弧、`42G` で行ジャンプ。`/` で検索、`n`/`N` で次・前。`v` `V` `Ctrl-v` で選択、`y` でコピー、`Ctrl+A` で全選択。
...
| `S` / `A` | AI にファイルの要約／コードの相談 |
| `:q` | このファイルを閉じる（最後の1つでビューアが閉じます） |
...
| `u` | 取り消し |
| `E`（`:edit`） | `$VISUAL` / `$EDITOR`（なければ nvim → vim → vi）で開き、戻ったら再読込 |
