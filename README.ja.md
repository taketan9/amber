...
**複数ファイルを一度に。** マークして `F3` を押すと全部開き、`F2` / `Shift+F2`、あるいはタイトルの `◂ ▸` クリックで行き来できます。各タブは、他を見ている間もカーソル位置・折りたたみ・未保存の編集をそれぞれ保持します。`Esc` で閉じるのは**見ているファイルだけ**で、最後の1つを閉じたときにビューアが閉じます。
