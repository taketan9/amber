...
| 81 | 2026-09-05 | 書く道具をキーボードの上に。よく使うものは今ので良い。見出しは複数回押すと `#` `##` `###`。使わないものは畳んでおく（表・リンク・ソースコード・取消線・水平線…） | 上のバーは **戻る・タイトル・保存・読む書く・⋯** の5つだけにして、道具は**キーボードが出ているときだけ**下に出す（キーボードが引っ込んでいるときの帯は「キーボードの上」ではなく、ただの画面の底）。見出しは**押すたびに深くなる** ── ボタンを3つ置くと、1つの考えに3つの名前が付く。タグ・通知・写真は上の ⋯ に残す（読んでいる間はキーボードが無く、通知を仕掛けるのは読み終えた直後だから）。**文字色は入れていない** ── Markdown に文字色は無く、書き方を決めると両前端がそれを覚えることになるので、先に相談したい | `ios/Cian/ContentView.swift ~ private func deepen\(` |
| 82 | 2026-09-05 | ノートを複数開いてスワイプすると、上のアイコンが一瞬増えて消える。ちらつきを消したい | `TabView` は**隣のページも生かしておく**ので、SwiftUI に2枚分のツールバーが渡り、入りきらない分が 「⋯」 に押し出される ── 増えて減っていたのは、その ⋯。**手前のページだけがツールバーを出す**ようにした | `ios/Cian/ContentView.swift ~ let current: Bool` |
