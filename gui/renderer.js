...
const MARKS = [
    ...
    [
        ...
        ['注記', '', cmdAlert],
        // **名前は「囲み」**（依頼 618・本人「文言はコードと重複しちゃうので
        // 別の何かを提案してほしい」）── 帯のすぐ上に「コード」という面の札が
        // あるので、同じ字を二つ置くと、押す前にどちらの話か分からない。
        // `` ``` `` は中でも前から「囲み」と呼んでいる（`codeOf` の註）。
        //
        // **中身は空で出す。** 見本の字を入れると（表がそうしている）、
...
/// **予定の中身を、そのまま出す小窓**（依頼 574・本人「マウスを重ねたり
