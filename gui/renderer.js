...
// 同じ1 つに訊いている。ここに写した瞬間、**同じ操作なのに Mac と iPhone で
...
/// 手当てを飛ばすと、既定の `<br>` が1 つだけ入り、**段落の末尾の `<br>`
...
/// 「アイコンと同じか」ではなかった。同じ1 つを渡せば、ずれようがない。
///
/// 128px の1 つで足りる ── いちばん大きい使い方（54）の2倍と、iPhone の
/// 38pt の3倍（114）を両方覆う。
function mark(size) {
...
/// `Intl.Collator` の使い回しでは 7ms）。並べ替えは一覧を組むたびにラン、
/// 一覧は保存のたびに組み直される。
...
///
/// `drawRead` が組んだときに札（`data-of`）を置き、**それ以外で文字が動いたら
...
///
/// だから途中に1 つ挟む: `webToMd` が均して、`blockToMd` が文字にする。
/// 画像の貼り付けは別に拾っている。
el('read').addEventListener('paste', (e) => {
    ...
    // **表示画面には、組んだ形のまま入れる。** 文字（`## 段取り`）を入れると
    ...
    switch (node.tagName) {
        ...
        case 'UL': case 'OL': {
            ...
            for (const li of node.children) {
                ...
                // 憶えていないもの（前の版で組んだ画面・画面の上で作った行）は、
                // これまで通りの形で書く。
...
function checkSoftReturn(box) {
    ...
    r.insertNode(br);
    // **行末に置いたら、詰め物をもう1 つ。** `<br>` が節の最後だと caret の
...
/// front matter を戻した、ファイルにある通りの1 つ。
///
...
function sweepMermaid(id) {
    for (const at of [id, 'd' + id]) {
        ...
        // 図そのもの**が消える（実際に消えて、図が1 つも出なくなった）。
...
/// ── 一つの考えに三つの名前を付けない、はボタンの話で、キーボードには当てはまらない
...
async function cmdCalendar() {
    ...
    drawRail();
    await hereAsk();
    // **開いたときに、いま置いてある1 つを読む**（依頼 476）。ここから先は
...
async function drawCal() {
    ...
    // **いつ時点の紙かを出す。** チームの予定は置き換わる1 つを読んで
...
/// **グループカレンダーを1 つ作る**（依頼 525・527）。
///
/// 押すのは一回。サインインも、カレンダーの許可も、主（Node）が面倒を見る
/// ── 使う人に段取りを踏ませない。
///
/// **いまの予定は一つも動かない。** 新しいカレンダーが1 つ増えるだけで、
...
/// 読む1 つの場所。**この環境の中だけに持つ**（ノートにも `.amber/` にも
/// 書かない ── あそこはフォルダと一緒に旅をする。会社のファイルの
/// 在りかが、家の環境にまで伝わることになる）。
let teamFile = '';
/// 最後に読めた1 つ。**月ごとには読み直さない**（依頼 476）── この紙は
/// 「今日から何日ぶん」の1 つで、月では分かれていない。月を替えるたびに
...
/// ひと月ぶん。**持っている1 つから選ぶだけ** ── ここではファイルを開かない。
...
/// 予定を登録するダイアログ（依頼 493）── **タイトル・終日・開始・終了**を1 つで。
...
async function findGroupCal() {
    // 会社向けの1 つには、グループという話そのものが無い（依頼 602）。
    if (OFFICE) return;
...
/// **会社向けの1 つでは、外のネットワークに触るものを出さない**（依頼 602・本人
...
const officeReady = (async () => {
    try {
        ...
        OFFICE = false;                       // 版を訊けない机では、ふつうの1 つ
    }
...
const CMDS = [
    ...
    { id: 'cal', name: 'カレンダー', sub: '予定と、その日のノートを1 つで', run: cmdCalendar },
    ...
    { id: 'team', name: 'チームの予定表を読む（CSV）', word: TEAM_WORD,
      sub: '別の道具が書き出した1 つを、人ごとに並べます', run: cmdTeam },
    ...
    // 会社向けの1 つにも出す ── ネットワークに出ず、手元の `.onepkg` を読むだけ。
...
/// 散らばっていて、**全部を一度に見る場所が無かった**。ここは1 つにして、
...
/// 琥珀の三つは amber が育ててきたものなので頭に残す。そのあとに cian のデスクトップ版の
...
function setTheme(name) {
    ...
    if (got) {
        // cian の配色は、変数を直に差す（琥珀の三つは `index.html` のラベルで）。
...
/// amber がするのは、そのフォルダに**マークを1 つ置くこと**だけ。マークはフォルダと
...
///
/// 四つ ── すべて／フォルダ一つ／タグの付いたもの／このノート1 つ。
...
async function cmdBackup() {
    ...
    if (state.open) {
        items.push({ name: 'このノート1 つ', sub: shortPath(state.open.path), value: ['note', state.open.path, rootOf(state.open.path)] });
    }
...
// **会社向けの1 つでは、訊きにも行かない**（依頼 602）── サインインの
...
/// ノートは1 つも動かさずに、二つ目が足せる。`at` が '' のもの（いちばん目）は、
    ...
    for (const s of plan.steps || []) {
        ...
        try {
            if (s.do === 'up') {
                ...
                // 隣に置く（失うよりよい）。隣に置いた1 つは、次の同期で新しく上がる。
...
async function cmdAbout() {
    ...
    await askPick('', [
        ...
        // ダイアログからはみ出していた ── 不具合を伝えるときに添える三つの
        // うち、いちばん読めない一つになっていた。
        ...(state.places.length
...
(async function boot() {
    ...
    // 会社向けの1 つでも最初の一瞬だけ Drive のラベルが出る。
    await officeReady;
