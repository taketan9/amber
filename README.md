...
**A ruler and a crosshair.** A column scale sits over the text — every fifth column marked, every tenth numbered — with the cursor's column picked out in it, and the cursor's line and column tinted underneath the syntax colours. Fixed-width records are most of what gets read on these machines, and counting characters by eye is what both are here to stop. `:ruler` puts them away.
