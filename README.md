...
**Several files at once.** Mark them and press `F3`: they all open, and `F2` / `Shift+F2` — or the `◂ ▸` in the title bar — walk between them. Each keeps its own cursor, its own folds and its own unsaved edits while another is on screen, and `Esc` closes the one you are reading rather than all of them — only the last one closes the viewer.
